...
输入
你的任务是写一个程序读取一些测试数据。每组测试数据表示一个计算机的文件结构。每组测试数据以'*'结尾，而所有合理的输入数据以'#'结尾。
...
对每一组测试数据，我们要先输出"DATA SET x:"，这里x是测试数据的编号（从1开始）。在两组测试数据之间要输出一个空行来隔开。
...
int main()
{
    priority_queue<string,vector<string>,greater<string> > findings[1010];
    int depth=1;
    bool flag=true;
    string name;
    int casenum=0;
    while(true)
    {
        ...
        {
            casenum++;
            cout<<"DATA SET "<<casenum<<":"<<endl<<"ROOT"<<endl;
            flag=false;
        }
        if(name[0]=='d')
        {
        ...
        if(name[0]=='f')
            findings[depth].push(name);
        if(name[0]==']'||name[0]=='*')
        {
            while(!findings[depth].empty())
            {
                for(int i=0;i<depth-1;++i)
                    cout<<"|     ";
                cout<<findings[depth].top()<<endl;
                findings[depth].pop();
            }
            --depth;
        }
